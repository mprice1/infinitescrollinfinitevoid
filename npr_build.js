...
NPR.init = function(gl) {
  ...
  // Convenient identity matrix.
  NPR.identity = [];
  mat4.identity(NPR.identity);
...
NPR.loadTexture = function(src, handler_or_callback, do_default_handler) {
  var gl = NPR.gl;
  var tex = gl.createTexture();
  tex.image = new Image();
  tex.image.onload = function() {
    tex.image.loaded = true;
    if (!handler_or_callback || do_default_handler) NPR.handleLoadedTexture(tex);
    if (handler_or_callback) handler_or_callback(tex);
  }
  tex.image.src = src;
  return tex;
}
...
NPR.Shader = function(vertex_src, fragment_src) {
  ...
  if (this.nMatrixUniform && nMatrix) {
    if (typeof nMatrix === 'boolean') {
      var nm = mat3.create();
      mat4.toInverseMat3(mvMatrix, nm);
      gl.uniformMatrix3fv(this.nMatrixUniform, false, mat3.transpose(nm));
    } else {
...
NPR.ColorShader = function() {
  ...
  this.setUniforms({
    uColor: [1, 0, 0, 1],
    uMVMatrix: NPR.identity,
    uPMatrix: NPR.identity
  });
}
